...
const ChangePasswordForm: React.FC<ChangePasswordFormProps> = ({ onSave, onClose, isSaving }) => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (errors[name] || (name !== 'oldPassword' && errors.confirmPassword)) {
            setErrors(prev => {
                const newErrors = { ...prev };
                delete newErrors[name];
                if (name !== 'oldPassword') delete newErrors.confirmPassword;
                return newErrors;
            });
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <fieldset disabled={isSaving}>
                <div>
                    <label htmlFor="oldPassword" className={labelClasses}>Contraseña Actual</label>
                    <input
                        type="password"
                        id="oldPassword"
                        name="oldPassword"
