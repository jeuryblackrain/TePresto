...
import { Client, Profile, Loan, LoanFrequency, PaymentType, Role, Route } from '../../types.ts';
...
const NewLoanForm: React.FC<NewLoanFormProps> = ({ onSave, onClose, isSaving }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        interest_rate: '',
        fixed_payment: '',
        frequency: LoanFrequency.MENSUAL,
        payment_type: PaymentType.INTERES,
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!profile) return;
            const { data: clientsData, error: clientsError } = await supabase.from('clients').select('*').eq('tenant_id', profile.tenant_id).order('name');
            if (clientsError) console.error("Error fetching clients", clientsError);
            else setClients(clientsData || []);

            const { data: routesData, error: routesError } = await supabase.from('routes').select('*').eq('tenant_id', profile.tenant_id).order('name');
    ...
    const validate = (): boolean => {
        ...
        const { client_id, employee_id, amount, term, payment_type, interest_rate, fixed_payment } = formData;

        if (!client_id) newErrors.client_id = "Debe seleccionar un cliente.";
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (validate()) {
            const { client_id, employee_id, amount, payment_type, interest_rate, fixed_payment, term, frequency, issue_date, route_id } = formData;
            ...
            <fieldset disabled={isSaving}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="client_id" name="client_id" value={formData.client_id} onChange={handleChange} required className={getInputClass('client_id')}>
                            <option value="">Seleccione un cliente</option>
                            {clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                        </select>
                        {errors.client_id && <p className={errorTextClasses}>{errors.client_id}</p>}
                    </div>
                    <div>
                        ...
                        <select 
                            ...
                            required 
                            className={`${getInputClass('employee_id')} disabled:bg-gray-100 dark:disabled:bg-gray-800 disabled:cursor-not-allowed`}
                            disabled={profile?.role === Role.EMPLOYEE}
                        >
                            {profile?.role === Role.ADMIN && <option value="">Seleccione un empleado</option>}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <div>
                        ...
                        <input type="text" inputMode="decimal" id="amount" name="amount" value={formData.amount} onChange={handleChange} required className={getInputClass('amount')} />
                ...
                {formData.payment_type === PaymentType.INTERES ? (
                    <div>
                        ...
                        <input type="text" inputMode="decimal" id="interest_rate" name="interest_rate" value={formData.interest_rate} onChange={handleChange} required className={getInputClass('interest_rate')} />
                    ...
                    <div>
                        ...
                        <input type="text" inputMode="decimal" id="fixed_payment" name="fixed_payment" value={formData.fixed_payment} onChange={handleChange} required className={getInputClass('fixed_payment')} />
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="route_id" name="route_id" value={formData.route_id} onChange={handleChange} className={baseInputClasses}>
